...
!---------------------------------------------------------------
  ...
  !
  integer :: mesh, i, icl, ii, mesh_exp
  integer :: nodes, hnodes, ncross, k, n_iter
  real(dp) :: xmax, dx, ddx12, norm, arg, djump, fac
  real(dp) :: eup, elw, e, e_correct
  ...
  !
     ...
     !analytical harmonic oscillator energy
     e_correct = nodes + 0.5_dp
     !fixed value of the grid discretization for plot
     mesh_exp = 500

  open(unit = 10, file = '5GLOBAL_ERROR.dat', action = 'write', status = "unknown")
  open(unit = 11, file = '5CONVERGENCE.dat', action = 'write', status = 'unknown')
  !
  ! this is the entry point for a new eigenvalue search
  !loop on discretization step
grid: do mesh = 100, 2000, 20
  e = 0.0
  ...
  !
     ...
     iterate: do k = 1, n_iter
        ...
        if ( n_iter > 1 ) then
           if (ncross /= nodes) then
              ...
              if ( k == 1 .AND. mesh == mesh_exp) then 
                write(11, *) 'Bisection                 Energy                 Nodes              Discontinuity'
                end if
              if (ncross > nodes) then
        ...
        if (mesh == mesh_exp) then
          write(11,*) e, ncross, nodes
          end if
        end if
        !
        ...
        if ( n_iter > 1 ) then
           ...
           if (mesh == mesh_exp)  write(11, * ) k, e, nodes, djump
            end if
    ...
    write(10,*) mesh, dx, e, e - e_correct
     !
