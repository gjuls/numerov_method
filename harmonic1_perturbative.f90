...
!---------------------------------------------------------------
  ...
  real(dp) :: xmax, dx, ddx12, norm, arg, ycusp, dfcusp, de, fac
  ...
  !output file
  open(unit = 14, file = 'PERTURBATION_ERROR.dat', action = 'write', status = "unknown")
  ...
  !
    do i = 0, mesh
     ...
     iterate: do k = 1, n_iter
        ...
        !
        if ( n_iter > 1 ) then
           !  calculate the new energy with perturbation theory
             ycusp = (y(icl-1)*f(icl-1)+f(icl+1)*y(icl+1)+10.0_dp*f(icl)*y(icl)) / 12.0_dp
             dfcusp = f(icl)*(y(icl)/ycusp - 1.0_dp)
...
! ---- convergence has been achieved (or it wasn't required) -----
   write(14,*) mesh, dx, e, abs(e - e_correct)
